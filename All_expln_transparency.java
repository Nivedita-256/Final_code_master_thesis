...
public class All_expln_transparency {
...
//		NPPhraseSpec result = nlgFactory.createNounPhrase("places");
   ...
   return output;
	...
	s1.addComplement(s2);
	NPPhraseSpec s3 = nlgFactory.createNounPhrase("has/have been rated highly by you, but have not been included in the sequence. This is because the ");
	
	//to compare
	
	//make the rating comapre 2D matrix of [places][people]
    String[] all = {"A","B","C","D","E","F","G","H","I","J"};
	
	int[][] rating_compare = new int[al3_size][3];
	...
	int myArray_length = myArray.size();
	int col_value;
	
	for(int j=0; j<myArray_length;j++) //places as rows
	{
		for(int i=0; i<all_length; i++) 
		{
			//System.out.println("myArray.get(j) "+myArray.get(j));
			if(all[i] == myArray.get(j))
			{
				col_value=i;
				
				rating_compare[j][0] = rating_val[0][col_value];//rating given by person 1 to place, row-j
				rating_compare[j][1] = rating_val[1][col_value];//rating given by person 2 to place, row-j
				rating_compare[j][2] = rating_val[2][col_value];//rating given by person 3 to place, row-j
			}
		}
	}
	...
	//System.out.println(rating_compare[1][1]);
	String[] names= {"John","Adam","Mary"};
	
//	NPPhraseSpec s4 = nlgFactory.createNounPhrase(names[others.get(0)]);
//	NPPhraseSpec s5 = nlgFactory.createNounPhrase(names[others.get(1)]);
//	CoordinatedPhraseElement obj = nlgFactory.createCoordinatedPhrase(s4, s5);
	
	s1.addComplement(s3);
//	s1.addPostModifier(obj);
//	s1.addComplement(s4);
    
	int n1=3;//myArray.size();
	ArrayList<String>[]  rated_low = new ArrayList[n1];
	
    for (int i = 0; i < n1; i++) { 
    	rated_low[i] = new ArrayList<String>(); 
    } 
	
    //create the rated_low "list of places" array for every person i
	for(int i=0;i<others.size();i++)// users other than name_index
	{
		for(int j=0;j<rating_compare.length;j++)//places in rating compare
		{
			if(rating_compare[j][others.get(i)] < 4)
			{
				rated_low[i].add(al[3].get(j));
				//System.out.println("rated_low "+i+rated_low[i]);
			}
		}
	}
	...
	//place I
	String place1 = null;
	NPPhraseSpec[] s4 = new NPPhraseSpec[others.size()]; //people
	for(int i=0; i< others.size();i++)
	{
		if(rated_low[i].size() > 0) 
		{
			//System.out.println("rated_low[i].size() "+i+rated_low[i].size());
			if(rated_low[i].size() == 1) 
			{
				place1=  singluar_string(myArray);
				place1 = place1.replace(".", " ");
				//System.out.println("inside al1size==1"+place1);
				NPPhraseSpec isratedlow =nlgFactory.createNounPhrase("is rated low by");
				NPPhraseSpec bynames =nlgFactory.createNounPhrase(names[others.get(i)]);
				isratedlow.addPreModifier(place1);
				isratedlow.addPostModifier(bynames);
				s4[0] = nlgFactory.createNounPhrase(isratedlow); //s4[i]
				s1.addComplement(s4[0]);
			}
			else if(rated_low[i].size() > 1) 
			{
				place1 = plural_string(myArray);
				place1 = place1.replace(".", " ");
				//System.out.println("inside al1size>1 "+place1);
				NPPhraseSpec areratedlow =nlgFactory.createNounPhrase("are rated low by");
				NPPhraseSpec bynames =nlgFactory.createNounPhrase(names[others.get(i)]);
				areratedlow.addPreModifier(place1);
				areratedlow.addPostModifier(bynames);
				s4[0]=nlgFactory.createNounPhrase(areratedlow);
				s1.addComplement(s4[0]);
			}
			if(s4[1]!=null)
			{
			CoordinatedPhraseElement obj =nlgFactory.createCoordinatedPhrase(s4[0],s4[1]);
			obj.setFeature(Feature.CONJUNCTION, "and");
			s1.addPostModifier(obj);
   ...
   return output;
	...
	s1.addComplement(s2);
	NPPhraseSpec s3 = nlgFactory.createNounPhrase("liked by you, will be visited later in the trip. This is due to the fact that");
	s1.addComplement(s3);
	...
	int[][] rating_compare = new int[al2_size][3];
	...
	int myArray_length = myArray.size();
	int col_value;
	NPPhraseSpec[] s5 = new NPPhraseSpec[rating_compare.length]; //people
	if(al2_size>0)
	{
		//make the rating comapre 2D matrix of [places][people]
		for(int j=0; j<myArray_length;j++) //places as rows
		...
		int[] users= {0,1,2};
		ArrayList<Integer>  others = new ArrayList<Integer>();
		//make an array that is contains names other than name_index
		for (int i=0;i<users.length;i++)
		{
			if (users[i]!=name_index)
			{
				others.add(users[i]);// create an array without name index
				//System.out.println("others "+others);
			}
		}
		//System.out.println(rating_compare[1][1]);
		...
		CoordinatedPhraseElement obj =null;//new CoordinatedPhraseElement[2]; //final s5's length is 2 .. number of users we are comparing
		int[] minval = new int[3];
		//System.out.println("rating_compare.length " + rating_compare.length);
		//function to calculate minimum
		int[] place_array=new int[3];

		
	//comparing rating value to decide band
		for(int j=0;j<rating_compare.length;j++)//rows
		{
			place_array = rating_compare[j];
			minval[j] = min(place_array,name_index);
			//System.out.println("minval[j]  "+minval[j] );
		    NPPhraseSpec word_place = nlgFactory.createNounPhrase("place");
			NPPhraseSpec placename= nlgFactory.createNounPhrase(al[2].get(j));
			
			s5[j]=nlgFactory.createNounPhrase(word_place);
			s5[j].addComplement(placename);
			
			if (minval[j] > 4 && minval[j] < 8)
			{
				s5[j] .addComplement("is rated moderately by"); 
				//System.out.println("names[others.get(j)] "+names[others.get(j)]);
				//s5[j].addComplement(names[others.get(j)]);
				for(int k=0;k<others.size();k++)
				{
					if (rating_compare[j][others.get(k)] ==minval[j])
					{
						s5[j].addComplement(names[others.get(k)]);	
					}
				}
			...
			{ 
				s5[j].addComplement("is rated low by"); 
				//s5[j].addComplement(names[others.get(j)]);
				for(int k=0;k<others.size();k++)
				{
					if (rating_compare[j][others.get(k)] ==minval[j])
			...
			{
				s5[j].addComplement("is rated highly by"); 
				//s5[j].addComplement(names[others.get(j)]);
				for(int k=0;k<others.size();k++)
				{
					if (rating_compare[j][others.get(k)] ==minval[j])
			...
			//low by john and moderately by mary
			
			if(s5.length>1)
			{
				obj =nlgFactory.createCoordinatedPhrase(s5[0], s5[1]);
				obj.setFeature(Feature.CONJUNCTION, "and");
				for (int i=2;i<s5.length;i++)
				{
				obj.addCoordinate(s5[i]);
				}
				s1.addComplement(obj);
			}
			else if (s5.length==1)
			{
			s1.addComplement(s5[0]);
			}
			
		}
		
	//System.out.println("rating_compare.length"+rating_compare.length);
//	NPPhraseSpec[] word_place = new NPPhraseSpec[rating_compare.length]; //place B, place J
//  NPPhraseSpec place= nlgFactory.createNounPhrase("place");
//	NPPhraseSpec is_rated= nlgFactory.createNounPhrase("is rated");
...
//		System.out.println(name+ " never recommended"+al[3]);
	...
	public static void main(String[] args)
		{
		All_expln_transparency aeg = new All_expln_transparency();
		
		int name_index=1; // change numbers here to choose the user -- 

		String a = aeg.algorithm_transparency(name_index);
		String b = aeg.like_recommended(name_index);
        String c = aeg.headsup();
		String d= aeg.never_recommended(name_index);
		String e = aeg.recommended_later(name_index);
		String f = aeg.dislike_recommended(name_index);
		String g = aeg.compromise();
		System.out.println("Transparent Explanation:  \n"+a+"\n"+b+"\n"+c+"\n"+d+"\n"+e+"\n"+f+"\n"+g);
